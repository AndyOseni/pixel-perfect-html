...
- Add `public/manifest.webmanifest` plus `icon-192.png` / `icon-512.png` and the manifest/theme-color/apple-touch-icon head tags for installability on both addresses.
- Offline support via `vite-plugin-pwa` (`generateSW`, `registerType: "autoUpdate"`, `/sw.js`, `injectRegister: null`, `devOptions.enabled: false`), registered only from one guarded wrapper module that refuses to register in dev, iframes, Lovable preview hosts, or with `?sw=off`, and unregisters stale workers there. Navigations use `NetworkFirst`; hashed same-origin assets use `CacheFirst`; `/~oauth` excluded. Offline reads fall back to the existing browser-local copies of members/savings data; writes and chat resume when back online.
- Note that offline behaviour only shows up in the published app, not in the editor preview.
